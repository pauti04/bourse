//! bourse server — accepts TCP connections and pipes orders through a
...
use bourse_core::hub::{Command, Hub, Submitter};
use bourse_core::matcher::Event;
use bourse_protocol::{ClientMessage, ServerMessage, decode_client, encode_server};
    ...
    R: AsyncReadExt + Unpin,
{
    ...
    r.read_exact(&mut body).await?;
    bourse_protocol::decode_server(&body)
        .map(Some)
