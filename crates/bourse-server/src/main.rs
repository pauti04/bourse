//! bourse server entry point. Binds to `127.0.0.1:9000` by default; pass
//! a single arg to override (`bourse-server 0.0.0.0:9000`).
//!
//! Logging is `tracing` + `tracing-subscriber` formatter. Set log level
//! via `RUST_LOG` (e.g. `RUST_LOG=info,bourse_server=debug`).
//!
...
use bourse_server::{Config, bind, serve_until};
...
async fn main() -> std::io::Result<()> {
    ...
    tracing::info!(%addr, "bourse-server listening");
